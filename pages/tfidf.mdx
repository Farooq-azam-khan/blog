...
The formula for calculating $\text{idf}$ is 

$$
\text{idf}(w) = \log(\frac{N}{\text{df}_t})
$$
where $\text{df}_t$ is the number of documents that contain the word $w$. E.g. $\text{df}('big') = 1$. Also, $N$ is the number of words in our list. For our toy example, $N=9$. 

# Putting it all Together, TFIDF
The last step is to get the $\text{tfidf}$. We simply multiply the $\text{tf}$ and the $\text{idf}$. 
$$
\text{tfidf}_{i,j}(\text{word}) = \text{tf}_{i,j} \cdot \text{idf}(\text{word})
$$
